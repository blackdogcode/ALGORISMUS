//
//  AI: DFS(Depth Frist Search)
//  https://www.hackerrank.com/challenges/pacman-dfs
//  Created by Sangchul, Song on 3/24/16.
...
public:
    ...
    bool isNotWall() { return type != WALL; }
    bool isFood() { return type == FOOD; }
    bool isPacman() { return type == PACMAN; }
...
void DFS(Node **maze, int packman_x, int packman_y, int food_x, int food_y, int row, int col);
...
int main(int argc, const char* argv[])
{
    ...
    // Depth First Search: from 'pacman' to 'food'
    DFS(maze, pacman_x, pacman_y, food_x, food_y, row, col);
...
inline void
DFS (Node **maze, int pacman_x, int pacman_y, int food_x, int food_y, int row , int col)
{
    Node* pacman = &maze[pacman_x][pacman_y];
    Node* food = &maze[food_x][food_y];
    stack<Node*> dfsStack; // FIFO: Explored Nodes (Discovered but, not yet Conquered)
    queue<Node*> dfsQueue; // FIFO: Expanded Nodes (Discovered and, also Conquered)
    
    pacman -> setExplored();
    dfsStack.push(pacman);
    while (!dfsStack.empty())
    {
        ...
        else
        {
            ...
            if (upNode -> isNotExplored() && upNode -> isNotWall())
            {
                ...
                upNode -> setExplored();
                dfsStack.push(upNode);
            }
            if (leftNode -> isNotExplored() && leftNode -> isNotWall())
            {
                leftNode -> setPredecessor(node);
                leftNode -> setExplored();
                dfsStack.push(leftNode);
            }
            if (rightNode -> isNotExplored() && rightNode -> isNotWall())
            {
                rightNode -> setPredecessor(node);
                rightNode -> setExplored();
                dfsStack.push(rightNode);
            }
            if (downNode -> isNotExplored() && downNode -> isNotWall())
            {
                downNode -> setPredecessor(node);
                downNode -> setExplored();
                dfsStack.push(downNode);
            }
        }
    }
    // Print explored nodes during executing dfs
    Node *exploredNode;
    cout << dfsQueue.size() << endl; // # of expanded nodes
    while (!dfsQueue.empty())
    {
        exploredNode = dfsQueue.front();
        exploredNode -> printPosition();
        dfsQueue.pop();
    }
    
    // Pinrt path from packman to food
    printPath(pacman, food);
}

// Print DFS path(not gurantee optimal) from src(packman) to des(food)
void
